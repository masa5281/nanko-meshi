...
export const FoodDesktopItem = ({
  ...
  return (
    <li className={`${className} flex p-4 bg-white rounded-lg shadow-sm shadow-shadow`}>
      ...
      <div className="flex flex-col flex-grow-2 justify-around">
        ...
        <div className="flex items-end">
          <div className="max-w-32 mr-5 px-1 bg-secondary rounded-lg text-white font-bold text-center">
            <p className="relative -top-1 text-2xl md:text-5xl">{calorieToFoodCount(food.calorie)}<span className="text-2xl">個分</span></p>
          </div>
          <div className="flex flex-col flex-grow-2 relative max-w-52 ml-auto">
            {
              foodPercentage(food.calorie) === 100 ? (
                <p className="text-lg md:text-2xl font-bold text-center"><span className="text-2xl md:text-4xl text-redBar">{foodPercentage(food.calorie)}%</span>以上消費</p>
              ) : (
